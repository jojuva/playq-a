...
//@ ```
//@
//@ s files. The wildcard `*` is accepted.
function _rm(options, files) {
  ...
  files.forEach(function(file) {
    ...
    // Recursively  existing directory
    if (stats.isDirectory() && options.recursive) {
...
//@ Arguments:
//@
//@ + `+N`: s the Nth directory (counting from the left of the list printed by dirs), starting with zero.
//@ + `-N`: s the Nth directory (counting from the right of the list printed by dirs), starting with zero.
//@
//@ Examples:
//@
//@ ```javascript
...
//@ ```
//@
//@ When no arguments are given, popd s the top directory from the stack and performs a cd to the new top directory. The elements are numbered from 0 starting at the first directory listed with dirs; i.e., popd is equivalent to popd +0. Returns an array of paths in the stack.
...
function rmdirSyncRecursive(dir, force) {
    ...
    if(currFile.isDirectory()) { // Recursive function back to the beginning
      ...
      if (force || isWriteable(file)) {
        try {
          ...
          error('could not  file (code '+e.code+'): ' + file, true);
        }
      }
    }
  ...
  try {
    ...
    error('could not  directory (code '+e.code+'): ' + dir, true);
  }
